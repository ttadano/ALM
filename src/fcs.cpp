                                        ...
                                        const bool store_zeros_in) const
{
    ...
    for (const auto &pair : pairs) {
        ...
        for (i1 = 0; i1 < nxyz; ++i1) {
            ...
            if (list_found.find(IntList(order + 2, &ind[0])) != list_found.end()) continue; // Already exits!
            ...
            for (isym = 0; isym < nsym_in_use; ++isym) {
                ...
                for (i2 = 0; i2 < nxyz; ++i2) {
                    ...
                    if (std::abs(c_tmp) > eps12) {
                        ...
                        if (list_found.find(IntList(order + 2, &ind_mapped[0])) == list_found.end()) {
                            list_found.insert(IntList(order + 2, &ind_mapped[0]));
