...
void Writer::write_input_vars(ALM *alm)
{
    ...
    std::cout << "  NSYM = " << alm->symmetry->nsym << "; PRINTSYM = " << alm->symmetry->printsymmetry
...
void Writer::write_force_constants(ALM *alm)
{
    ...
    for (order = 0; order < maxorder; ++order) {
        ...
        if (alm->fcs->nequiv[order].size() > 0) {
...
void Writer::write_displacement_pattern(ALM *alm)
{
    ...
    for (order = 0; order < maxorder; ++order) {
        ...
        for (auto it = alm->displace->pattern_all[order].begin();
...
void Writer::write_misc_xml(ALM *alm)
{
    ...
    for (unsigned int ui = 0; ui < alm->fcs->nequiv[0].size(); ++ui) {
        ...
        child.put("<xmlattr>.pairs",
                  boost::lexical_cast<std::string>(alm->fcs->fc_table[0][ihead].elems[0])
                  + " " + boost::lexical_cast<std::string>(alm->fcs->fc_table[0][ihead].elems[1]));
    ...
    if (alm->interaction->maxorder > 1) {
        ...
        for (unsigned int ui = 0; ui < alm->fcs->nequiv[1].size(); ++ui) {
            ...
            ptree &child = pt.add("Data.ForceConstants.CubicUnique.FC3",
                                  double2string(alm->fitting->params[k]));
            child.put("<xmlattr>.pairs",
                      boost::lexical_cast<std::string>(alm->fcs->fc_table[1][ihead].elems[0])
                      + " " + boost::lexical_cast<std::string>(alm->fcs->fc_table[1][ihead].elems[1])
                      + " " + boost::lexical_cast<std::string>(alm->fcs->fc_table[1][ihead].elems[2]));
